class FormComponent {
  ...
  constructor(selector, formatErrors, onSuccess) {
    ...
    if (htmlElement === null) {
      throw new Error(
        `failed FormComponent instance creation:\n'${selector}' is not valid selector.`
      );
    }
  ...
  initFields() {
    this.#fields = Array.from(
      this.#htmlElement.querySelectorAll("[name]")
  ...
  initErrorHtmlElements() {
    this.#errorHtmlElements = Array.from(
      this.#htmlElement.querySelectorAll("[field-error]")
    ).reduce(
      (prevErrorElements, errorElement) => ({
        ...prevErrorElements,
  ...
  clearFieldsErrors() {
    ...
    Object.values(this.#errorHtmlElements).forEach(
